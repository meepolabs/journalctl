...
verification (see ``journalctl/middleware/auth.py``). Both paths consult
the same ``app.state.journalctl_gateway_secret`` and apply the same
...
async def resolve_user_id(
    ...
    if settings.auth.trust_gateway:
        ...
        if sig_present or sig_required:
            ...
            gateway_secret = getattr(request.app.state, "journalctl_gateway_secret", None)
        ...
        # (c) Hydra bearer
            ...
            from journalctl.auth.hydra import HydraInvalidToken, HydraUnreachable
